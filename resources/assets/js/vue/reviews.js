var vueApp = new Vue({
    ...
    methods: {
        ...
        fetchItems: function () {
            ...
            this.$http.get('ajax/review/index', function (items) {
        ...
        selectListItem: function (item) {
            console.log(item.id);
            window.location = "/reviews/" + item.id;
        },
        toggleRead: function (item,ev) {
            ...
            this.$http.post('ajax/reviews/' + item.id + '/toggleRead').success(function (blah) {
                ...
                alert('Sorry, There was an error updating the review, please refresh the page and try again.');
            });
        },         
